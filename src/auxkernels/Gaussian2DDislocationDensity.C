...
validParams<Gaussian2DDislocationDensity>()
{
  ...
  params.addClassDescription(
      "Compute dislocation density based on gaussian 2D distribution");
...
Gaussian2DDislocationDensity::computeValue(){
  ...
  Real x = _q_point[_qp](0); // local x coordinate of bundle fed into 2D gaussian distribution equation
  Real y = _q_point[_qp](1); // local y coordinate of bundle fed into 2D gaussian distribution equation
  Real A = _N / (h * _sigma_x * std::sqrt(2)); // Amplitude of dislocation bundle

  if (x >= _x_min && x <= _x_max && y >= _y_min && y <= _y_max)
    return A * std::exp(-(((x - _x_center) * (x - _x_center)) / (std::sqrt(2) * _sigma_x) + ((y - _y_center) * (y - _y_center)) / (std::sqrt(2) * _sigma_y)));
